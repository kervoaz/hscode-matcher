...
A Java Spring Boot REST API that maps free-text user input to HS (Harmonized System) customs classification codes. It is customer-facing — end users filling GEODIS shipping forms describe their goods in plain language (in French, English, or German) and receive the correct HS code, even when the input is imprecise or informal.
