...
Suggested root package: `com.zou.hs.matcher` (adjust only if org standards require a different groupId).

| Package | Purpose (Phase 1) |
|---------|-------------------|
| `com.zou.hs.matcher` | `@SpringBootApplication` entrypoint |
| `com.zou.hs.matcher.domain` | `HsEntry`, `Language`, `SearchResult`, `HierarchyContext` |
| `com.zou.hs.matcher.ingestion` | Empty placeholder (`package-info.java` only) |
| `com.zou.hs.matcher.search` | Empty placeholder (`package-info.java` only) |
| `com.zou.hs.matcher.api` | Empty placeholder (`package-info.java` only) |
...
- `hscode-matcher-api/src/main/java/com/zou/hs/matcher/Application.java`
- `hscode-matcher-api/src/main/resources/application.properties`
- `hscode-matcher-api/src/main/java/com/zou/hs/matcher/ingestion/package-info.java`
- `hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/package-info.java`
- `hscode-matcher-api/src/main/java/com/zou/hs/matcher/api/package-info.java`
...
**Description:** Implement **`Language`**, **`HsEntry`**, **`HierarchyContext`**, and **`SearchResult`** in **`com.zou.hs.matcher.domain`**. Use `public` top-level types (records/enum). Add brief Javadoc on `HsEntry` for `code` normalization and `level` meanings. Do **not** add Spring annotations to domain types. Keep **`List<HsEntry> siblings`** as `java.util.List`; null-safety policy: document whether `siblings` may be empty vs null (recommend **non-null empty list** for “no siblings” to simplify Phase 4 callers).
...
- `hscode-matcher-api/src/test/java/com/zou/hs/matcher/domain/HsEntryTest.java` (or one `DomainModelTest.java` — prefer focused class names)
