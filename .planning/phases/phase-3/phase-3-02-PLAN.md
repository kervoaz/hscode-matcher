...
files_modified:
  ...
  - hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingEngine.java
  - hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingStore.java
  - hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingSearcher.java
  - hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingConfig.java
  - hscode-matcher-api/src/main/resources/onnx/tokenizer.json
  - hscode-matcher-api/src/test/java/com/zou/hs/matcher/search/embedding/EmbeddingEngineTest.java
  - hscode-matcher-api/src/test/java/com/zou/hs/matcher/search/embedding/EmbeddingSimilarityIT.java
autonomous: true
requirements:
  - R3
  - R8
...
must_haves:
  ...
  artifacts:
    - path: "hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingEngine.java"
      provides: "Tokenize + ONNX infer + mean pool + L2 normalize; wraps OrtSession via AtomicReference"
      exports: ["embed(String text): float[]"]
    - path: "hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingStore.java"
    ...
    - path: "hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingSearcher.java"
    ...
    - path: "hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingConfig.java"
...
From src/main/java/com/zou/hs/matcher/domain/HsEntry.java:
```java
...
From src/main/java/com/zou/hs/matcher/domain/Language.java:
```java
public enum Language { FR, EN, DE }
```

From src/main/java/com/zou/hs/matcher/ingestion/NomenclatureRegistry.java:
```java
...
From src/main/java/com/zou/hs/matcher/search/model/ScoredHit.java (created by Plan 01, same wave):
```java
...
`com.zou.hs.matcher.search.model` — reference it from there. If Plan 01 has not run yet,
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    **EmbeddingConfig** (package `com.zou.hs.matcher.search.embedding`):
    ```java
    @Configuration
    ...
    **EmbeddingEngine** (package `com.zou.hs.matcher.search.embedding`):
    ...
    **EmbeddingEngineTest** (package `com.zou.hs.matcher.search.embedding`):
  ...
  <verify>
    <automated>cd /c/Users/zou/dev/ZOU/HSCODE/hscode-matcher-api && rtk ./mvnw test -pl . -Dtest=EmbeddingEngineTest -q 2>&1 | tail -20</automated>
  </verify>
  <done>
...
<task type="auto" tdd="true">
  ...
  <action>
    **EmbeddingStore** (package `com.zou.hs.matcher.search.embedding`):
    ...
    **EmbeddingSearcher** (package `com.zou.hs.matcher.search.embedding`):
    ...
    **EmbeddingSimilarityIT** (package `com.zou.hs.matcher.search.embedding`):
  ...
  <verify>
    <automated>cd /c/Users/zou/dev/ZOU/HSCODE/hscode-matcher-api && rtk ./mvnw test -pl . -Dtest="EmbeddingEngineTest,EmbeddingSimilarityIT" -q 2>&1 | tail -20</automated>
  </verify>
  <done>
...
```bash
grep -n "destroyMethod" hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingConfig.java
```
Must show `destroyMethod = ""`.

Verify inputNames are discovered at init, not hardcoded:
```bash
grep -n "input_ids" hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingEngine.java
```
...
```bash
grep -n "l2Normalize\|l2normalize" hscode-matcher-api/src/main/java/com/zou/hs/matcher/search/embedding/EmbeddingEngine.java
```
</verification>
